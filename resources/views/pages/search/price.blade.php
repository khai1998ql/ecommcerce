...
if(Session::has('orderby')){
    if($min != $max){
        $product = DB::table('products')
            ...
            ->whereBetween('products.selling_price', [$min, $max])
        ...
        $product = DB::table('products')
            ->join('subcategories', 'products.subcategory_id', 'subcategories.id')
            ->where('products.selling_price', '>', $min)
            ->where('products.status', 1)
            ->distinct('products.id')
            ->select('products.*', 'subcategories.subcategory_name')
            ->orderBy('selling_price', Session::get('orderby'))
                ...
                <div class="col-lg-12">
                    <div class="breadcrumbs-menu">
                        <ul>
                            ...
                            <li><a href="" class="active">Tìm kiếm sản phầm có giá từ: <span style="color: red">{{ $min }} đên {{ $max }}</span></a></li>
                        </ul>
                    </div>
                </div>
            </div>
